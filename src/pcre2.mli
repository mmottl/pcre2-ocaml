...
(** [regexp_or ?limit ?depth_limit ?iflags ?flags ?chtables patterns] like
    {!regexp}, but combines [patterns] as alternatives (or-patterns) into one
    regular expression. *)
...
val unsafe_pcre2_dfa_match :
  ...
  workspace:int array ->
  unit
(** [unsafe_pcre2_dfa_match flags rex ~pos ~subj_start ~subj offset_vector
     callout ~workpace]. You should read the C-source to know what happens. If
    you do not understand it - {b don't use this function!} *)
